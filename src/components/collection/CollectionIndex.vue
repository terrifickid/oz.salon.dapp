<template>
  <div>
    <CollectionSingleModal :artwork="collectionSingleItem" v-if="collectionSingleItem"
      @close="collectionSingleItem = null" />
    <div class="grid grid-cols-12 border-t border-black py-2 gap-x-3 md:gap-x-5 cursor-pointer"
      v-for="(item, index) in collection" :key="index" @click="collectionSingleItem = item">
      <div class="col-span-4 md:hidden">
        <div class="bg-gray-100 p-5">
          <div class="w-full aspect-square bg-contain bg-no-repeat bg-center bg-gray-100"
            :style="{ backgroundImage: 'url(' + image(item) + ')' }"></div>
        </div>
      </div>
