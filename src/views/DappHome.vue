  ...
  <div class="container-fluid mx-auto">
    ...
    <div class="grid grid-cols-12 gap-10 px-10">
      ...
      <div class="col-span-8 py-10 flex">
        ...
        <template v-if="userAddress">
          <template v-if="!isKyc"> <FormKyc /></template>
          <template v-if="isKyc"> <FormApply /> </template>
        </template>
      </div>
    </div>
  </div>
...
export default {
  name: "DappHome",
  components: { DappHeader, FormKyc, FormApply },
  computed: {
    isKyc() {
      var found = false;
      this.members.forEach((item) => {
        if (item.wallet == this.userAddress) found = true;
      });
      return found;
    },
    userAddress() {
